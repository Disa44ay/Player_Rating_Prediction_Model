...
def transform_and_process_data(df):
    ...
    initial_transformer = joblib.load('pipelines/30_12_25-13_04_06_custom_attribute_object.pkl')
    df = initial_transformer.transform(df)    

    transformer = joblib.load('pipelines/30_12_25-13_04_06_full_transformer.pkl')
    transformed_data = transformer.transform(df)

    model = joblib.load('models/30_12_25-13_10_52_best_model.pkl')
    predicted_value = model.predict(transformed_data)

    target_transformer = joblib.load('pipelines/30_12_25-13_04_06_target_pipeline.pkl')
